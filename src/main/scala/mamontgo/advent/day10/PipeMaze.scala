...
object PathBuilder {
  ...
  def count(maze: PipeMaze): Int = {
    ...
    val res = findLongestLoopPath(maze)
  ...
  def findLongestLoopPath(maze: PipeMaze): Seq[Seq[Adjacent]] = {
    val mapRes: Seq[Seq[Adjacent]] = findAllPaths(maze)

    // calculate the longest path in loop formation
    val max: Int = getMaxSizeOfLoopingPath(mapRes)

    // get the longest looping paths
    mapRes.filter(p => p.size == max)
  }

  def findAllPaths(maze: PipeMaze): Seq[Seq[Adjacent]] = {
    maze.start().adjacent().map(x => find(x))
  }
...
object PipeMaze {
  ...
  def getAdjacentPositions(position: Position): Seq[Position] = {
    adjacents(position).map(i => Position(Vector(), i._1, i._2))
  }

  def adjacents(position: Position): Seq[(Int, Int)] = {
    Seq((position.row - 1, position.col), (position.row + 1, position.col), (position.row, position.col - 1), (position.row, position.col + 1)).filter(i => i._1 >= 0 && i._2 >= 0)
  }
  def findAdjacent(m: Maze, position: Position): Seq[Adjacent] = {
    adjacents(position).filter(i => i._2 < m.head.size && i._1 < m.size).map(i => (m(i._1)(i._2), i)).filter(i => {
  ...
  def findStart(m: Maze): Position = {
    findFirstPosition(START, m)
  }

  def findFirstPosition(d: Char, m: Maze): Position = {
  ...
  def findAllPositions(d: Char, m: Maze): Seq[Position] = {
    m.foldLeft((0, Seq[Position]())) {
      (i, v) => {
        val x: (Int, Seq[Position]) = v.foldLeft((0, i._2)) {
          (i2, v2) => {
            if (v2 == d) {
              (i2._1 + 1, Position(m, i._1, i2._1) +: i2._2)
            } else (i2._1 + 1, i2._2)
          }
        }
        (i._1 + 1, x._2)
...
object Position {
  ...
  def min(p: Position) = Position(Vector(), p.row, p.col)
