...
case class MapDirections(dirs: Seq[Dir], map: TreeMap) {
  ...
  @tailrec
  final def testConcurrent(iteration: Long, list: Seq[ItemSearchCollect], matches: Seq[Seq[Long]]): Long = {
    val res: Seq[Any] = Await.result(
      Future.sequence(
        Seq(processConcurrent(list), Future {
  ...
  @tailrec
  final def searchAll(s: ItemSearchAll): ItemSearchAll = {
    val res = Ftils.foldUntil[Dir,ItemSearchAll](dirs, s, i => i.found, { (i, v: Dir) => {
      i.test(i.next.map(key => {
        get(key).get(v)
      }))
    }})
  ...
  @tailrec
  private final def search(s: ItemSearch): ItemSearch = {
  ...
  @tailrec
  final def breakingSearch(attempts: Long, s: ItemSearchCollect): ItemSearchCollect = {
    val res = findItems(s)
    if(attempts == 0) {
      res
    } else {
...
object MapDirections {
  ...
  def matches(res: Seq[Seq[Long]]): Option[Long] = {
    if(res.isEmpty)
      ...
      Ftils.foldUntil[Long, Option[Long]](res.head, None:Option[Long], x => x.isDefined, (_, v) => if(res.tail.forall(v1 => v1.contains(v))) Some(v) else None)
  }
